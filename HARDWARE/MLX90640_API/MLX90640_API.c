...
static CCMRAM float g_param_extract_scratch[768];
...
#define MLX90640_FRAME_READ_RETRY_LIMIT 2
#define MLX90640_READY_WAIT_TIMEOUT_ERROR (-9)

static uint32_t mlx90640_ready_period_us(uint8_t refreshRate)
{
    switch(refreshRate)
    {
        case FPS1HZ:
...
static uint32_t mlx90640_ready_wait_timeout_us(uint8_t slaveAddr)
{
    int refreshRate;
    uint32_t periodUs;

    refreshRate = MLX90640_GetRefreshRate(slaveAddr);
    if(refreshRate < FPS1HZ || refreshRate > FPS32HZ)
    {
        refreshRate = RefreshRate;
    }

    periodUs = mlx90640_ready_period_us((uint8_t)refreshRate);

    return periodUs + (periodUs / 2UL) + 5000UL;
}

static void mlx90640_ready_wait_timer_init(void)
{
...
static uint32_t mlx90640_elapsed_us(uint32_t startCycle)
{
...
static int mlx90640_wait_frame_ready(uint8_t slaveAddr, uint16_t *statusRegister)
{
    uint16_t localStatusRegister = 0U;
    uint16_t dataReady = 0U;
    uint32_t waitStartCycle;
    uint32_t timeoutUs;
    int error;

    timeoutUs = mlx90640_ready_wait_timeout_us(slaveAddr);
    mlx90640_ready_wait_timer_init();
    waitStartCycle = DWT->CYCCNT;

    /* Bound the frame-ready wait so a stalled sensor does not block the
     * thermal pipeline indefinitely. */
    while(dataReady == 0U)
    {
        error = MLX90640_I2CRead(slaveAddr, 0x8000, 1, &localStatusRegister);
        if(error != 0)
        {
            return error;
        }

        dataReady = localStatusRegister & 0x0008U;
        if(dataReady != 0U)
        {
            break;
        }

        if(mlx90640_elapsed_us(waitStartCycle) >= timeoutUs)
        {
            return MLX90640_READY_WAIT_TIMEOUT_ERROR;
        }
    }

    if(statusRegister != 0)
    {
        *statusRegister = localStatusRegister;
    }
...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    error = mlx90640_wait_frame_ready(slaveAddr, &statusRegister);
    if(error != 0)
    {
        return error;
    }
    dataReady = statusRegister & 0x0008U;
        
    /* Retry a bounded number of times if the sensor still reports the
     * frame-ready flag after the clear command. */
    while(dataReady != 0 && cnt < MLX90640_FRAME_READ_RETRY_LIMIT)
    { 
...
//    }
//    
//    gain = params->gainEE / gain; 			//  获取Kgain  11.2.2.4
//  
////------------------------- To calculation -------------------------------------    
...
void MLX90640_CalculateTo(uint16_t *frameData, const paramsMLX90640 *params, float emissivity, float tr, float *result)
{
    ...
    // 【优化】常数增加 f 后缀，避免自动升级为 64位 double
    ta4 = (ta + 273.15f);
    ta4 = ta4 * ta4;
    ta4 = ta4 * ta4;
...
//------------------------- To calculation -------------------------------------    
    ...
    for( int i = 0; i < 2; i++)
    {
        ...
        {
            irDataCP[i] = irDataCP[i] - 65536.0f;
        }
        irDataCP[i] = irDataCP[i] * gain;
    }
    
    // 【优化】提取出针对全帧恒定不变的温漂常数，避免在循环中运算 768次
    float ta_minus_25 = ta - 25.0f;
    float vdd_minus_3_3 = vdd - 3.3f;
    float cp_comp = (1.0f + params->cpKta * ta_minus_25) * (1.0f + params->cpKv * vdd_minus_3_3);
    
    irDataCP[0] = irDataCP[0] - params->cpOffset[0] * cp_comp;
    
    if( mode ==  params->calibrationModeEE)
    {
        irDataCP[1] = irDataCP[1] - params->cpOffset[1] * cp_comp;
    }
    else
    {
        irDataCP[1] = irDataCP[1] - (params->cpOffset[1] + params->ilChessC[0]) * cp_comp;
    }
...
//------------------------- Pixel Loop -----------------------------------------
    for( int pixelNumber = 0; pixelNumber < 768; pixelNumber++)
    {
        ...
        {    
            ...
            // 【优化】复用前面提出来的常数，节省2次减法和2次乘法
            irData = irData - params->offset[pixelNumber] * (1.0f + kta * ta_minus_25) * (1.0f + kv * vdd_minus_3_3);
            
            if(mode != params->calibrationModeEE)
            {
                irData = irData + params->ilChessC[2] * (2.0f * (float)ilPattern - 1.0f) - params->ilChessC[1] * (float)conversionPattern; 
            }                       
            ...
            alphaCompensated = SCALEALPHA * alphaScale / params->alpha[pixelNumber];
            alphaCompensated = alphaCompensated * (1.0f + params->KsTa * ta_minus_25); // 同样复用 ta_minus_25
                        
            Sx = alphaCompensated * alphaCompensated * alphaCompensated * (irData + alphaCompensated * taTr);
            
            // 【DSP硬件加速】替换原始的 sqrt(sqrt(Sx))
            if (Sx > 0.0f) {
                arm_sqrt_f32(Sx, &temp_sqrt);
                arm_sqrt_f32(temp_sqrt, &Sx);
            } else {
                Sx = 0.0f; // 防护无效数据
            }
            Sx = Sx * params->ksTo[1];            
            ...
            // 【DSP硬件加速】第一次 To 估算
            arg_sqrt = irData / (alphaCompensated * (1.0f - params->ksTo[1] * 273.15f) + Sx) + taTr;
            if (arg_sqrt > 0.0f) {
                arm_sqrt_f32(arg_sqrt, &temp_sqrt);
                arm_sqrt_f32(temp_sqrt, &To);
                To = To - 273.15f; 
            } else {
                To = ta; // 兜底保护
            }                     
            ...
            // 【DSP硬件加速】最终 To 修正计算
            arg_sqrt = irData / (alphaCompensated * alphaCorrR[range] * (1.0f + params->ksTo[range] * (To - params->ct[range]))) + taTr;
            if (arg_sqrt > 0.0f) {
                arm_sqrt_f32(arg_sqrt, &temp_sqrt);
                arm_sqrt_f32(temp_sqrt, &To);
                To = To - 273.15f;
...
float MLX90640_GetTa(uint16_t *frameData, const paramsMLX90640 *params)
{
    ...
    vdd = MLX90640_GetVdd(frameData, params);
    //LCD_ShowFloatNum2(0,20,vdd,4,BLACK,WHITE,16);
    ptat = frameData[800];
    if(ptat > 32767)
    {
        ptat = ptat - 65536;
...
void ExtractAlphaParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    uint8_t accRemScale;
    float *alphaTemp = g_param_extract_scratch;
    float temp;
...
void ExtractKtaPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    uint8_t ktaScale2;
    uint8_t split;
    float *ktaTemp = g_param_extract_scratch;
    float temp;
...
void ExtractKvPixelParameters(uint16_t *eeData, paramsMLX90640 *mlx90640)
{
    ...
    uint8_t kvScale;
    uint8_t split;
    float *kvTemp = g_param_extract_scratch;
    float temp;
